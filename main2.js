...
function _chart(partition,data,d3,width,color,arc,format,radius)
{
  ...
  path.filter(d => d.children)
      .style("cursor", "url('sandwich.png'), auto")
      .on("click", clicked);
...
d3.scaleOrdinal([`#EA8A81`, `#DC7027`, `#301008`, `#F4D8AE`, `#B6C48E`])
)}
...
export function clara(runtime, observer) {
  const main = runtime.module();
  function toString() { return this.url; }
  const fileAttachments = new Map([
    ["flare-2.json", {url: new URL("./files/Clara_repas_data.json", import.meta.url), mimeType: null, toString}]
  ]);
