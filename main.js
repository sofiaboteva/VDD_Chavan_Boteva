...
function _chart(partition,data,d3,width,color,arc,format,radius)
{
  ...
  path.filter(d => d.children)
      .style("cursor", "url('verre.png'), auto")
      .on("click", clicked);
...
d3.scaleOrdinal([`#EA8A81`, `#F4D8AE`, `#301008`, `#DC7027`, `#B6C48E`])
)}
...
export function sofia(runtime, observer) {
  const main = runtime.module();
  function toString() { return this.url; }
  const fileAttachments = new Map([
    ["flare-2.json", {url: new URL("./files/Sofia_repas_data.json", import.meta.url), mimeType: null, toString}]
  ]);
